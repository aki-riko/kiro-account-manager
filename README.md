...
### 🏗️ v1.9.2 - 2026-06-17 — Linux ARM64 架构支持

> 本版本新增 **Linux ARM64** 架构完整支持，现在树莓派、ARM 服务器等设备也可以原生运行本应用。
...
#### 🖥️ macOS 兼容
- **修复**: macOS 启动后窗口不显示 — 新增 3 秒兜底机制，前端未能及时触发窗口显示时自动强制显示。

#### 🔒 UA 对齐
- **修复**: API 请求 User-Agent 与真实 Kiro IDE 完全对齐 — management API 使用 `1.0.0` + `codewhispererruntime`，streaming API 使用 `1.0.39` + `codewhispererstreaming`。
