...
function getProviderMeta(provider: string, t: any): ProviderMeta {
  const providers: Record<string, ProviderMeta> = {
    ...
    BuilderId: {
      ...
      icon: <span className="text-[#ff9900] font-bold text-base">aws</span>,
    },
    Enterprise: {
      name: t('login.idc'),
      icon: <span className="text-[#ff9900] font-bold text-base">aws</span>,
    ...
    <div className="h-full flex flex-col items-center justify-center glass-main relative overflow-hidden p-6">
      <div className="relative z-10 w-full max-w-sm flex flex-col items-center">
        ...
        <div className="flex items-center gap-2.5 mb-6">
          ...
          <span className="text-2xl font-bold text-foreground tracking-wide">KIRO</span>
        </div>

        {/* 副标题 */}
        <h1 className="text-sm text-muted-foreground text-center mb-5">{t('login.subtitle')}</h1>

        {/* 错误条 */}
        {error && (
          <div className="w-full mb-4 p-3 rounded-lg text-sm text-center bg-destructive/10 text-destructive border border-destructive/20">
            {error}
          </div>
        )}
        ...
        {/* 协议说明 */}
        <div className="text-[11px] text-muted-foreground text-center leading-relaxed mt-6 max-w-[400px]">
          {t('login.agreement')}{' '}
