...
2026-07-14 最终代理代码与本机安装 Kiro 的官方模型握手复核：

- 显式 lifecycle 测试启动本机安装的 Kiro 0.12.333，并使用最终代理代码等待真实 `Management/ListAvailableModels` 操作。
- 测试观测事件只会在代理的 `reqwest.send().await` 已取得官方上游 HTTP 响应后产生，因此已坐实官方 IDE 的精确模型 RPC 穿过本地白名单到达上游。
- 假 KSK 未被上游接受，符合测试预期；测试耗时 21.45 秒并完整清理。
