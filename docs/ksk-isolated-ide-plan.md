...
- 当前功能提交：`2703d6a fix(ksk-ide): 禁止与正式 Kiro 并行启动`
...
2. 正式 Kiro 完全退出后，KAM 启动一个使用独立目录的 Kiro IDE 实例。
...
8. 正式 Kiro 的用户目录、登录态和扩展目录不受隔离实例修改；首版不承诺两个 Kiro 实例同时运行。
...
KAM 前端
  ...
  -> 创建隔离 HOME / user-data / extensions
  -> 写入无权限占位 token 和本地 endpoint 配置
  -> 启动独立 Kiro.exe
...
`session-id` 使用随机 UUID。目录不得包含 KSK。
...
### 7.3 隔离 settings.json

监听器成功绑定并取得动态端口后，才生成 `settings.json`。
...
- `HOME` 指向隔离 `home`。
- `--user-data-dir` 指向隔离 `user-data`。
- `--extensions-dir` 指向隔离 `extensions`。
- `--new-window` 强制独立窗口。
...
状态：代码已完成；并行运行已收紧为禁止，等待正式 Kiro 关闭后的最终实机回归。

- 创建会话目录。
- 原子写占位 token 和 settings。
- 验证生成文件中不存在输入 KSK。
- 使用子进程专属环境启动 Kiro。
- 跟踪 PID，不使用全局 kill。
- 停止后清理目录并报告部分失败。
...
状态：进行中。已完成完整自动化门禁与一次假 KSK 生命周期测试；真实 KSK 聊天尚未验收。
...
处理：记录实际进程树；如果 `std::process::Child` 不能代表完整实例，改为 Windows Job Object 或经验证的 PID 树跟踪。不得退回按映像名 kill。
...
- loopback 动态监听。
- 认证头替换。
- KSK runtime `profileArn` 删除。
- 原生 EventStream 字节流透传。
- mock 上游单元测试。

在这些测试通过前，不启动真实 Kiro，不接入前端，不使用真实 KSK。
